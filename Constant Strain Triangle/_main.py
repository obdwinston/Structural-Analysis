...
Np = [2445, 3103, 1934, 1682, 2949, 2948, 2682, 2898, 3177, 3250, 3330, 3307, 3118,
      3181, 2998, 2427, 2476, 3026, 3086, 43, 96]  # point force node - [Np1, Np2, ...]
Fp = []  # point force [N] - [[Fpx, Fpy], ...]

p = 1e7  # surface force [Pa]
L = .04  # surface length [m]
l = L/(len(Np) - 1)  # element length [m]
Fs = p*l*t/2  # point force equivalent [N]
...
for i in range(n_nodes):
    px = nodes[i][0]
    py = nodes[i][1]
    plt.text(px, py, i, c='r', horizontalalignment='center',
...
S = Sv  # change stress (Sa, Sv, S1, S2)
f = 10  # displacement amplification factor

cmap = mpl.cm.jet
norm = mpl.colors.Normalize(vmin=min(S), vmax=max(S))
ticks = np.linspace(min(S), max(S), 10)
fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), cax=ax2,
             orientation='horizontal', label='Stress [Pa]', ticks=ticks)
...
for i in range(n_cells):
    ...
    color = cmap(norm(S[i]))
    ax1.plot([x1, x2, x3, x1], [y1, y2, y3, y1],
             c='k', linewidth=0.5, zorder=1)
    ax1.fill([x1n, x2n, x3n, x1n], [y1n, y2n, y3n, y1n],
