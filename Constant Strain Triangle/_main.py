...
mesh = 'test.su2'  # mesh file

E = 200e9  # Young's modulus [Pa]
nu = .25  # Poisson's ratio
t = 25e-3  # thickness [m]

Np = [603, 355]  # point force node [Np1, Np2, ...]
Fp = [[100000., 0.], [0., -100000.]]  # point force [[Fpx, Fpy], ...]
...
for i in range(n_cells):
    ...
    S[i] = .5*(sx + sy)  # average stress
    S1[i] = .5*(sx + sy) + m.sqrt((.5*(sx - sy))**2 + txy**2)  # maximum principal stress
    S2[i] = .5*(sx + sy) - m.sqrt((.5*(sx - sy))**2 + txy**2)  # minimum principal stress
    Sv[i] = m.sqrt(sx**2 + sy**2 - sx*sy + 3*txy**2)  # von Mises stress
...
for i in range(n_cells):
    ...
    color = cmap(norm(S[i]))
    plt.plot([x1, x2, x3, x1], [y1, y2, y3, y1], c='k', linewidth=0.5, zorder=1)
    plt.fill([x1n, x2n, x3n, x1n], [y1n, y2n, y3n, y1n], facecolor=color, edgecolor=None, zorder=2)
    # plt.fill([x1n, x2n, x3n, x1n], [y1n, y2n, y3n, y1n], facecolor=color, edgecolor='b', linewidth=0.5, zorder=2)
